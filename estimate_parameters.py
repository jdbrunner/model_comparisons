...
if mono:
    print("estimate_parameters.py: estimating single species parameters.")
    mono_params = {}
    ...
    # print(mono_params_df)
    # print((mono_params_gore - mono_params_df)/mono_params_df)
    #Very ^
    ...
    print("estimate_parameters.py: estimating single species parameters done.")
    mono_params_df.to_csv('mono_parameters_new.csv')

    #Plot of the fitted curves
    show_me_2 = True
    if show_me_2:
        print("estimate_parameters.py: plotting single species models.")
        for i in mono_data.keys():
            fig, ax = plt.subplots()
            ax.set_title(i)
            ax.plot(mono_data[i],'*')
            log_fun = make_logistic_fun(mono_data[i].mean(axis = 1)[1])
            ax.plot(np.linspace(1,5,100),log_fun(np.linspace(1,5,100),mono_params_df.loc[i,'r'],mono_params_df.loc[i,'K']))
            fig.savefig('parameter_fit_plots/parameter_estimation_mono/'+i)
            plt.close()


if pairw:
    mono_params_df = pd.read_csv('mono_parameters_new.csv', index_col = 0)
    ...
    inter_params_dict = {}
    pair_outcome_dict = {}
    print("estimate_parameters.py: estimating pair parameters.")
    for k in pair_data_scld.keys():#our own parameter fitting
        print("estimate_parameters.py: estimating pair parameters:" + k)
        spes = k.split('_')
        pars,ou = model_fit_wrapper(pair_data_scld[k], [mono_params_df.loc[spes[0],'r'], mono_params_df.loc[spes[1],'r']], numtris = nt,masterbound = 100.0)
    ...
    #Creates a dataframe with pair outcomes according to model fitting (pair_outs_fit)
    #Creates a dataframe of fit pairwise interaction parameters (interact_params)
    print("estimate_parameters.py: estimating pair parameters done")
    # pair_outs_fit.to_csv('pair_outcomes_new.csv')
    # interact_params.to_csv('lotka_volterra_fitted_new.csv')
    ...
    if show_me:#plotting of system with fitted parameters
        ...
        print("estimate_parameters.py: plotting pair models.")
        for i in pair_data.keys():
            ...
            for j in pair_data_scld2[i].index.levels[0]:
                ...
                ax3.set_xlabel('')
                ax4.set_xlabel('')
            fig.savefig('parameter_fit_plots/parameter_estimation_pair/'+i)
            plt.close()

        print("estimate_parameters.py: plotting trio models.")
        for spes in real_outs.index:
            ...
            fig.savefig('parameter_fit_plots/parameter_estimation_trio/'+spes[0]+spes[1])
